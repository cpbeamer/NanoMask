...
pub fn main() !void {
    ...
    // --- Stage 4: Unified Single-Pass Scanner (all patterns) ---
    {
        ...
        // Seed mixed PII at intervals: email, phone, CC, IP, healthcare
        ...
        while (pos + 25 <= payload_size) {
            const sample = samples[sample_idx % samples.len];
        ...
        var run: usize = 0;
        while (run < iterations) : (run += 1) {
            const result = try scanner.redact(payload, flags, allocator);
            allocator.free(result);
        }
        const elapsed_ns = timer.read();
        const total_bytes = payload_size * iterations;
        const mb_per_sec = (@as(f64, @floatFromInt(total_bytes)) /
            @as(f64, @floatFromInt(elapsed_ns))) * 1_000_000_000.0 / (1024.0 * 1024.0);
        println("Stage 4 | Pattern Scanner     | {d:>8.1} MB/s | {d:>4} iter x {d} bytes (all 5 patterns)", .{
            mb_per_sec, iterations, payload_size,
        });
    }
